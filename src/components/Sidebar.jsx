...
import { ButtonsDirective, TooltipComponent } from '@syncfusion/ej2-react-popups';
...
const Sidebar = () => {
  ...
  const normalLlink = 'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-gray-700 text-md dark:text-gray-200 dark:hover:text-black hover:bg-light-gray m-2';
            ...
            <div key={ item.title }>
              ...
              {item.links.map((link) => (
                <NavLink
                  ...
                  key={link.name}
                  onClick={() => {}}
                  className={({ isActive }) => isActive ? activeLink : normalLlink}
                >
                  {link.icon}
